...
def setup_store(
    ...
    for key, sub_config in settings_dict.items():
        if key.startswith("[") and key.endswith("]"):
            ...
            raise cabinetConfigError(f"Pyramid settings unknown key {key_prefix}.{key}")
...
def get_handler(key_prefix: str, settings_dict: Dict) -> StorageHandlerBase:
    ...
    try:
        ...
        raise cabinetConfigError(f"Pyramid settings bad value for {name}")
    ...
    try:
        ...
        raise cabinetConfigError(f"Pyramid settings bad args for {name}: {err}")
...
def get_filter(key_prefix: str, settings_dict: Dict) -> FilterBase:
    ...
    try:
        ...
        raise cabinetConfigError(f"Pyramid settings bad value for {key_prefix}")

    kwargs = {key: decode_kwarg(value) for key, value in settings_dict.items()}
    try:
        return filter_cls(**kwargs)
    except Exception as err:
        raise cabinetConfigError(f"Pyramid settings bad args for {key_prefix}: {err}")
...
def decode_kwarg(value: Any) -> Any:
    ...
    if (value.startswith("[") and value.endswith("]")) or (
        ...
        try:
            ...
            raise cabinetConfigError(f"Pyramid settings bad value {value}: {err}")
