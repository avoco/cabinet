...
from vectum import AsyncStorageHandlerBase, FileItem
...
class S3Handler(AsyncStorageHandlerBase):
    ...
    async def _validate(self) -> Optional[Awaitable]:
        ...
        if aioboto3 is None:
            raise vectumConfigError("aioboto3 library required but not installed.")
    ...
    async def test_credentials(self):
        ...
        item = self.get_item(
            ...
            data=BytesIO(b"Credential test run from the vectum library."),
        )
